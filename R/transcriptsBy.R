                        ...
                        order_by_exon_rank=TRUE)
{
    ...
    ##################################################################### steve
    if (splicing_in_join) {
      selectClause <- paste(selectClause, ', transcript.tx_name')
      fromClause <- paste(fromClause, ', transcript')
      whereClause <- paste(whereClause, 'AND transcript._tx_id=splicing._tx_id')
    }
    ##################################################################### steve
    ...
    ##################################################################### steve
    .split <- if (by == 'tx') 'tx_name' else paste(by, "_id", sep="")
    ##################################################################### steve
    
    split(grngs, ans[[.split]])
}
